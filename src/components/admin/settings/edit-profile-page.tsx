...
export function EditProfilePage({ userEmail, fullName, bio }: Props) {
  ...
  async function handleChangePassword(e: React.FormEvent) {
    ...
    if (!passwordValid)       { setPassMsg({ type: "err", text: "Hasło nie spełnia wymagań." }); return; }
    if (password !== confirm) { setPassMsg({ type: "err", text: "Hasła nie są identyczne." }); return; }
    ...
    startPassTransition(async () => {
      if (current && userEmail) {
        ...
        if (signInError) { setPassMsg({ type: "err", text: "Obecne hasło jest nieprawidłowe." }); return; }
      }
    ...
    <div className="min-h-dvh bg-[#080810] text-white">
      ...
      <header className="border-b border-white/10 px-6 py-4">
        <div className="mx-auto flex max-w-lg items-center gap-4">
          ...
          <h1 className="text-base font-semibold">Edytuj profil</h1>
        </div>
      </header>

      <main className="mx-auto max-w-lg space-y-8 px-6 py-8">
        {userEmail && (
          <p className="text-sm text-zinc-500">Konto: <span className="text-zinc-300">{userEmail}</span></p>
        )}

        {/* ── Profile section ── */}
        <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
          <h2 className="mb-5 text-sm font-semibold text-zinc-300">Dane profilu</h2>
          <form onSubmit={handleSaveProfile} className="flex flex-col gap-4">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-zinc-400">
                Opis / nazwa organizacji <span className="text-zinc-600">(opcjonalnie)</span>
              </label>
              <textarea value={description} onChange={(e) => setDesc(e.target.value)}
                placeholder="Krótki opis Ciebie lub Twojej firmy…"
                rows={3}
                className={inputCls + " resize-none"} />
            </div>

            {profileMsg && (
              <p className={`rounded-xl px-4 py-2.5 text-sm ${profileMsg.type === "ok" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
                {profileMsg.text}
              </p>
            )}

            <button type="submit" disabled={profilePending}
              className="cursor-pointer rounded-2xl bg-violet-600 py-3 text-sm font-semibold text-white hover:bg-violet-500 transition-colors disabled:opacity-50">
        ...
        {/* ── Password section ── */}
        <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
          <h2 className="mb-5 text-sm font-semibold text-zinc-300">Zmiana hasła</h2>
          <form onSubmit={handleChangePassword} className="flex flex-col gap-4">
            ...
            {passMsg && (
              <p className={`rounded-xl px-4 py-2.5 text-sm ${passMsg.type === "ok" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
                {passMsg.text}
              </p>
            )}

            <button type="submit" disabled={passPending || !passwordValid || (!!confirm && confirm !== password)}
              className="cursor-pointer rounded-2xl bg-gradient-to-r from-indigo-500 to-violet-500 py-3 text-sm font-semibold text-white disabled:opacity-50 transition-opacity">
