...
export function ProfileAccountMenu({ userEmail }: { userEmail?: string | null }) {
  ...
  useEffect(() => {
    ...
    function close(e: MouseEvent) {
      if (!(e.target as Element).closest("[data-profile-account-menu]")) {
        setOpen(false);
      }
    }
        ...
        <div className="absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-white/10 bg-[#13111f] shadow-xl">
          {userEmail && (
            <div className="border-b border-white/10 px-4 py-3">
              ...
              <p className="mt-0.5 break-all text-sm font-medium text-white">
                {userEmail}
              </p>
            </div>
          )}
          ...
          <div className="border-t border-white/10 px-3 py-2">
            <SignOutButton className="w-full rounded-xl border border-red-500/20 py-2 text-sm text-red-400 transition-colors hover:bg-red-500/10" />
          </div>
        </div>
      )}
    </div>
